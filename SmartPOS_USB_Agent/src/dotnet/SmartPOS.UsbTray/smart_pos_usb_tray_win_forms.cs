// SmartPOS.UsbTray.WinForms — минимальный WinForms-трей-клиент для локального агента
// Author: Разработчик суперпрограмм
// Сборка: dotnet new winforms -n SmartPOS.UsbTray && заменить Program.cs на этот файл (target net6.0-windows);
//         добавьте в .csproj: <UseWindowsForms>true</UseWindowsForms>
// Назначение: показывает иконку в трее, опрашивает http://127.0.0.1:8765/api/status, даёт Preflight, Recycle, Export ZIP.
// Безопасность: POST-запросы передают X-API-Key из переменной окружения SMARTPOS_USB_APIKEY (если задана).
